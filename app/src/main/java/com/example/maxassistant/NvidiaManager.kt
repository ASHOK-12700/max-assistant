...
class NvidiaManager : AiProvider {
    ...
    override suspend fun askAi(question: String): Result<String> = suspendCancellableCoroutine { continuation ->
        ...
        call.enqueue(object : Callback {
            override fun onFailure(call: Call, e: java.io.IOException) {
                android.util.Log.e("MAX_AI", "NVIDIA request failed: ${e.message}")
                continuation.resume(Result.failure(e))
            }

            override fun onResponse(call: Call, response: Response) {
                val code = response.code
                android.util.Log.d("MAX_AI", "NVIDIA HTTP status: $code")
                ...
                if (response.isSuccessful && body != null) {
                    try {
                        ...
                        android.util.Log.d("MAX_AI", "NVIDIA response received and parsed")
                        // Add AI response to history
                        addMessage("assistant", content)
                        continuation.resume(Result.success(content))
                    } catch (e: Exception) {
                        android.util.Log.e("MAX_AI", "NVIDIA parse error: ${e.message}")
                        continuation.resume(Result.failure(e))
                    }
                } else {
                    android.util.Log.e("MAX_AI", "NVIDIA error response: $code - $body")
